...
export class RescueStationEditModalComponent {
	...
	updateSignedInStation(): void {
		...
		// if the protocol data is dirty, we want to make sure it is valid as it is an optional form for updates
		// first check if some fields are filled, if not, we mark the form as pristine to handle the case where a user can enter a value, validates it, deletes it => should not be required
		if (
