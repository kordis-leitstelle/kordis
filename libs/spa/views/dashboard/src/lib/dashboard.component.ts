...
import { ProtocolComponent } from '@kordis/spa/protocol';
...
export class DashboardComponent {
	...
	readonly name$ = this.authService.user$.pipe(
		map((u) => u?.firstName + ' ' + u?.lastName),
	);
	private readonly router = inject(Router);
	private readonly modal = inject(NzModalService);
	private readonly observabilityService = inject(OBSERVABILITY_SERVICE);

	logout(): void {
		this.authService.logout();
	}

	showReportErrorModal(): void {
		this.observabilityService.openErrorModal();
	}
