...
/*
 * Populates the units and alert groups with the assigned units by their ids
 */
@Injectable()
export class UnitsPopulateService {
	constructor(private readonly queryBus: QueryBus) {}

	async getPopulatedUnitsAndAlertGroups(
		unitIds: string[],
		alertGroups: {
			alertGroupId: string;
		...
		const assignedAlertGroups = await this.getPopulatedAlertGroups(
			alertGroups,
			units,
		);
	...
	private async getPopulatedAlertGroups(
		alertGroupsToEnrich: {
			alertGroupId: string;
			assignedUnitIds: string[];
		}[],
